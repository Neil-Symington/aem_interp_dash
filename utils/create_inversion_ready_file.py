...
infile = r"C:\Users\u77932\Documents\EFTF2\SW\data\existing\AEM\110284_Data_Package\AEM\netcdf\AUS_10013_Musgrave_EM.nc"

mus = aem_utils.AEM_data(name = 'Musgraves AEM data',
                              system_name = 'SkyTEM312',
                              netcdf_dataset = netCDF4.Dataset(infile))

infile = "E:\GA\AEM\EM\AUS_10021_DalyR_EM.nc"

daly = aem_utils.AEM_data(name = 'Daly AEM data',
                              system_name = 'SkyTEM312',
                              netcdf_dataset = netCDF4.Dataset(infile))

# use inbuilt functions to calculate the noise
mus.calculate_noise("low_moment_Z-component_EM_data", noise_variable = "low_moment_Z_component_noise",
                       multiplicative_noise = 0.03)

# use inbuilt functions to calculate the noise
daly.calculate_noise("low_moment_Z-component_EM_data", noise_variable = "low_moment_Z_component_noise",
                       multiplicative_noise = 0.03)

mus.calculate_noise("high_moment_Z-component_EM_data", noise_variable = "high_moment_Z_component_noise",
                       multiplicative_noise = 0.03)

# Noise is root of sum of the squares
lm_noise = mus.low_moment_Z_component_noise
hm_noise = mus.high_moment_Z_component_noise

# Now divide this by the absolute value of the data to get the relative uncertainty

lm_runc = lm_noise/np.abs(mus.data['low_moment_Z-component_EM_data'][:].data)
hm_runc = hm_noise/np.abs(mus.data['high_moment_Z-component_EM_data'][:].data)
...
mask_array = np.zeros(mus.data.dimensions['point'].size, dtype=np.bool)
...
write_inversion_ready_file(mus.data, outfile, nc_variables,
