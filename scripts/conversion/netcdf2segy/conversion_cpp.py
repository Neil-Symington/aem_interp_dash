...
# get the number of samples so we can compute the layer point probability from the histogram
nsamples = np.float(d.variables['log10conductivity_histogram'][0][0].sum())


histogram_var = 'interface_depth_histogram'
max_depth = 350.
...
for line in lines:
    ...
    data_dict = aem2segy.get_sorted_line_data(d, line, vars = ['fiducial', histogram_var, 'easting',
                                                               'northing', 'elevation'], sort_on = 'fiducial')

    # calculate probability from the histogram
    data_dict['cpp'] = np.divide(data_dict[histogram_var],nsamples)
    ...
    for i in range(len(stream_cond)):
        interp_dat = aem2segy.interpolate_layer_data(data_dict['layer_top_depth'], z_new,
                                                     data_dict['cpp'][i], data_dict['elevation'][i],
                                                     max_depth, datum)
    ...
    outfile = os.path.join(outdir, str(line) + '_cpp.segy')
